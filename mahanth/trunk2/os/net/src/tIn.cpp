...
const u8 TInterface::macAllHost[6] = { 0x01, 0x00, 0x5e, 0x00, 0x00, 0x01 };

TInterface::TInterface(es::NetworkInterface* networkInterface) :
...
bool TReceiver::input(InetMessenger* m, Conduit* c)
{
...
bool TReceiver::output(InetMessenger* m, Conduit* c)
{
...
bool TInReceiver::input(InetMessenger* m, Conduit* c)
{
    IPHdr* iphdr = static_cast<IPHdr*>(m->fix(sizeof(IPHdr)));

    esReport("first line %x-%x-%x\n",iphdr->verlen,iphdr->tos,iphdr->len);
    esReport("second line %x-%x\n",iphdr->id,iphdr->frag);
    esReport("third line %x-%x-%x\n",iphdr->ttl,iphdr->proto,iphdr->sum);
    esReport("fourth line %x\n",iphdr->src.addr);
    esReport("fifth line %x\n",iphdr->dst.addr);
}

bool TInReceiver::output(InetMessenger* m, Conduit* c)
{
    static const u8 zero[6] = { 0, 0, 0, 0, 0, 0 };
    ASSERT(m);
...
bool TARPReceiver::output(InetMessenger* m, Conduit* c)
{
    static const u8 zero[6] = { 0, 0, 0, 0, 0, 0 };
    ASSERT(m);
